...
def bus():
    print(time)
    initial_answer = input("You hop on the bus! Do you want to sit down and relax your puppies or stand up near the front? Enter 'sit' or 'stand' ").lower()
    if initial_answer == "sit":
        print("Oh no! You got too comfy and missed your stop. Now you have to get off and walk 2 blocks back up the street!")
        return [walk_to_bus_stop, missed_stop, interview()]
    elif initial_answer == "stand":
        print("The ride was a little shaky but you held on and finally made it to your interview!")
        return [walk_to_bus_stop, stand, interview()]

def uber():
    print(time)
    answer = input("Uh oh! You accidently hit uber pool! Do you want to get in, wait for another, hop on your bike, or head to the bus stop? Choose 'In', 'Wait', 'Bike', 'Bus':  ").lower()
    if answer == "in":
...
def commute():
    print(time)
    initial_answer = input("Now its time to head to the interview! How are you going to get there? 'Uber', 'Bike', 'Bus'?: ").lower()
